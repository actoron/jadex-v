...
export class RegistryTableElement extends BaseElement 
{
	...
	subscribeToCoordinator()
	{
		...
		this.callid = jadex.getIntermediate("coordinatorapi/subscribe", res =>
		{
	...
	loadDefaultStyle() 
	{
	    return this.loadStyle("./coordinatorapi/style.css");
	}
	...
	dragElement(element) 
	{
		...
		const isEdgeClick = (e, edge = 20) => 
		{
			const rect = element.getBoundingClientRect();
			const x = e.clientX - rect.left;
			const y = e.clientY - rect.top;
			return (
				x <= edge || x >= rect.width - edge ||
				y <= edge || y >= rect.height - edge
			);
		};
		...
		element.addEventListener("pointerdown", md);
	}
