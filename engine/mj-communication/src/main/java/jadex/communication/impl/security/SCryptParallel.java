...
public class SCryptParallel
{
    ...
    private static byte[] MFcrypt(byte[] P, byte[] S, final int N, final int r, int p, int dkLen)
    {
        ...
        try
        {
            ...
            int tc = BLen / MFLenWords;
            final CyclicBarrier bar = new CyclicBarrier(tc + 1);
            for (int BOff = 0; BOff < BLen; BOff += MFLenWords)
            {
            	final int[] fB = B;
            	final int fBOff = BOff;
