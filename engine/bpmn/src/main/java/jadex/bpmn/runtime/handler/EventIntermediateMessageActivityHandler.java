...
public class EventIntermediateMessageActivityHandler extends DefaultActivityHandler
{
	 ...
	 */
	public ComponentIdentifier getCid(Object cid, ComponentIdentifier parent)
	{
	...
	protected void sendMessage(final MActivity activity, final IComponent instance, final ProcessThread thread)
	{
		...
		{
			List<ComponentIdentifier> tmp = new ArrayList<>();
			for(Object r: SReflect.getIterable(rec))
			{
				tmp.add(getCid(r);//, instance.getId().getParent()));
			}
			receivers = tmp.toArray(new ComponentIdentifier[tmp.size()]);
		}
		else if(rec!=null)
		{
			receivers = new ComponentIdentifier[]{getCid(rec)};//, instance.getId().getParent())};
		}
		...
		if(message instanceof Map)
		{
			...
			else 
			{
				...
				if(r instanceof ComponentIdentifier)
					receivers = new ComponentIdentifier[]{(ComponentIdentifier)r};
			}
	 ...
	 *  @param thread	The process thread.
	 */
	protected void receiveMessage(final MActivity activity, final IComponent instance, final ProcessThread thread)
	{
		thread.setWaiting(true);
