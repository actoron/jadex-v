...
//		System.out.println("gloaldelehandler: "+SUtil.arrayToString(args));
		Future<Object> ret = (Future<Object>)FutureFunctionality.getDelegationFuture(method.getReturnType(), new FutureFunctionality(agent.getLogger()));
	...
	//			}
				...
				goal = new RGoal(agent, mgoal, null, null, vals, null, null);
			}
	...
	//				((IBDIAgentFeature)bdif).dropGoal(fgoal);
			...
			ret = (Future<Object>)FutureFunctionality.getDelegationFuture(method.getReturnType(), func);
			SFuture.avoidCallTimeouts(ret, agent);
			
			final Future<Object> fret = ret;
