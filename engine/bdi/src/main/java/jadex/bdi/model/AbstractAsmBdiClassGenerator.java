...
//					allcz.add(INonUserAccess.class);
					...
					for(MethodInfo mi: allms.keySet())
					{
						...
						{
							nl.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "jadex/bdi/runtime/impl/BDIAgentFeature", "getBDIAgentFeature", "(Ljadex/core/IComponent;)Ljadex/bdi/features/IBDIAgentFeature;", false));
						}
...
//							nl.add(new InsnNode(Opcodes.SWAP));
						...
						if(m.getDeclaringClass().equals(IBDIAgentFeature.class))
						{
							nl.add(new MethodInsnNode(Opcodes.INVOKEINTERFACE, "jadex/bdi/features/IBDIAgentFeature", mnode.name, mnode.desc, true));
						}
...
//						System.out.println(info);
	...
	protected boolean isAgentOrCapa(String annotationDescription)
	{
		return (annotationDescription.indexOf("Ljadex/micro/annotation/Agent;")!=-1
	    	|| annotationDescription.indexOf("Ljadex/bdi/annotation/Capability;")!=-1);
	}
	...
	{
		return annotationDescription.indexOf("Ljadex/bdi/annotation/Goal;")!=-1;
	}
	...
	{
		return annotationDescription.indexOf("Ljadex/bdi/annotation/Plan;")!=-1;
	}
	...
	protected boolean isPlanClass(ClassNode classNode) 
	{
		...
		if(visibleAnnotations!=null)
		{
			for(AnnotationNode an: visibleAnnotations)
			{
				if("Ljadex/bdi/annotation/Plan;".equals(an.desc))
				{
					result	= true;
					break;
				}
	...
	protected boolean isGoalClass(ClassNode classNode) 
	{
		...
		if(visibleAnnotations!=null)
		{
			for(AnnotationNode an: visibleAnnotations)
			{
				if("Ljadex/bdi/annotation/Goal;".equals(an.desc))
				{
					result	= true;
					break;
				}
