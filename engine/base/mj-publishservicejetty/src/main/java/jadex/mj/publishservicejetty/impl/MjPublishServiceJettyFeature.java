...
public class MjPublishServiceJettyFeature extends MjPublishServiceFeature
{
	// Hack constant for enabling multi-part :-(
	private static final MultipartConfigElement MULTI_PART_CONFIG = new MultipartConfigElement(System.getProperty("java.io.tmpdir"));

    /** The servers per service id. */
	protected static Map<IServiceIdentifier, Server> sidservers;

    /** The servers per port. */
    protected static Map<Integer, Server> portservers;
	...
	protected MjPublishServiceJettyFeature(MjComponent self)
	{
		super(self);
	}
...
//	        IFuture<MultiCollection<String, MappingInfo>> fut = evaluateMapping(serviceid, info);
        IFuture<PathManager<MappingInfo>> fut = evaluateMapping(serviceid, info, getClassLoader());
        		...
        		try
                {
                    ...
                    ContextHandler ch = new ContextHandler()
                    {
                            ...
                            throws IOException, ServletException
                        {
                        	if(service==null)
                        	{
                        		service = getComponent().getExternalAccess().scheduleStep((IComponent agent) ->
                        		{
