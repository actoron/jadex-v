...
import jadex.remoteservice.impl.remotecommands.IMethodReplacement;
import jadex.remoteservice.impl.remotecommands.ProxyReference;
import jadex.remoteservice.impl.remotecommands.ProxyInfo;
...
//			return pi;
		...
		else if((args==null || args.length==0) && "hashCode".equals(method.getName()))
		{
			...
			id	= id instanceof IService ? ((IService)id).getServiceId()
					: id instanceof IComponentHandle ? ((IComponentHandle)id).getId()
					: id;
		...
		else if(args!=null && args.length==1 && "equals".equals(method.getName()) && Object.class.equals(method.getParameterTypes()[0]))
		{
			return pr.getRemoteReference().getTargetIdentifier().equals(
				...
				: args[0] instanceof IComponentHandle ? ((IComponentHandle)args[0]).getId() : args[0]);
		}
...
//		CallAccess.resetNextInvocation(); // done in feature
		...
		if(!comp.getFeature(IExecutionFeature.class).isComponentThread())
		{
			...
			{
				public IFuture<Object> execute(Component comp)
				{
					return ((IRemoteExecutionFeature)comp.getFeature(IRemoteExecutionFeature.class))
