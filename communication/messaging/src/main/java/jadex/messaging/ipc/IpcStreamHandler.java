...
public class IpcStreamHandler
{
	...
	protected void open()
	{
		ServerSocketChannel tserverchannel = null;
		Path socketpath = socketdir.resolve(""+gpid.pid());
		try
		{
			...
			tserverchannel = ServerSocketChannel.open(StandardProtocolFamily.UNIX);
			tserverchannel.configureBlocking(true);
			tserverchannel.bind(socketaddress);
		}
		catch(IOException e)
		{
		...
		ServerSocketChannel serverchannel = tserverchannel;
