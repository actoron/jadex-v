 ...
 */
public class ExecuteProcessThread implements IThrowingConsumer<IComponent> //IConditionalComponentStep<Void>
{
	/** The process thread. */
...
//			System.out.println("not exe: "+thread.getInstance().getComponentIdentifier().getLocalName()+" "+thread);
	 ...
	 */
	//public IFuture<Void> execute(IComponent ia)
	public void accept(IComponent component) throws Exception
	{
		if(!isValid())
			return;
