...
//	@Goal(recur=true, posttoall=true) // Goal flags: variation 4
	...
	class MaintainBatteryLoaded
	{
		...
		{
			return self.getChargestate()>=0.7; // Everything is fine as long as the charge state is above 20%, otherwise the cleaner needs to recharge.
		}
...
//		IChargingstation	chargingstation	= stations.iterator().next();	// from Exercise C0
	...
	{
		// Start an agent
		IComponentManager.get().create(new CleanerBDIAgentC2());
