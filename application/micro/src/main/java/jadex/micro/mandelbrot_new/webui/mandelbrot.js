...
class MandelbrotElement extends HTMLElement 
{
	...
	paint()
	{
		...
		if(this.calculating && this.range!=null)
		{
			...
			ctx.drawImage(canvas2, srange.x, srange.y, srange.width, srange.height, 0, 0, width, height);
		}
		...
		if(!this.calculating && this.range!=null)
		{
			...
			if(rratio<bratio)
			{
				...
				ctx2.fillRect(srange.x+offset, srange.y, -offset, srange.height+1);
				ctx2.fillRect(srange.x+srange.width, srange.y, -offset, srange.height+1);
			}
	...
	getMethodPrefix() 
	{
		...
		return '/webjcc/invokeServiceMethod?servicetype='+'jadex.micro.mandelbrot_new.IDisplayService';
	}
		...
		return `
			...
			<div id="settings" class="dragable fitcontent fitcontentheight">
				<div class="grid">
					...
					<!-- Adapts the canvas draw area to its component size, should be normal behavior -->					
					...
					<!-- Adapts the size of a newly calculated image to the size of the canvas. -->
					...
					<input name="fill" id="fill" class="checkboxleft" type="checkbox" onclick="let host = window['getHost'](event.target); host.paint()">
				</div>
			
				<div class="floatright margintop">
					<button class="fitcontent jadexbtn" onclick="let host = window['getHost'](event.target); host.paint()">Repaint</button>
					<button class="fitcontent jadexbtn" onclick="let host = window['getHost'](event.target); host.resetSettings(event)">Reset</button>
					<button class="fitcontent jadexbtn" onclick="let host = window['getHost'](event.target); host.generateArea(event)">Generate</button>
				</div>
			</div>
