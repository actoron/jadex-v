...
public interface IRuleSystemService
{
	 ...
	 */
	public default void notifyEvent(EventType type, String data)
	{
		System.out.println("notify Event: "+type+" ("+data+")");
	}
	
	/**
	 *  Register an LLM prompt that is triggered by the given event type and source.
	 */
	@Tool("Register a rule for an LLM prompt to be executed later, when the given event occurs.")
	public default void createRule(@P("The event type") EventType type, @P("The LLM prompt to be executed") String prompt)
	{
