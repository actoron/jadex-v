...
public class SensorActuator
{
	 ...
	 */
	public ITerminableFuture<Void> moveTo(ILocation location)
	{
	 ...
	 */
	public ITerminableFuture<Void> moveTo(double x, double y)
	{
		TerminableFuture<Void> ret = new TerminableFuture<>();
		
		ret.then(Void -> System.out.println("moveTo end")).catchEx(ex -> System.out.println("moveToEnd ex"));
		...
		{
			System.out.println("move aborted: "+target);
			reached.setExceptionIfUndone(new RuntimeException("move aborted"));
			target = null;
		});
		reached.delegateTo(ret);
...
//				t.printStackTrace();
	 ...
	 */
	public IFuture<Void> recharge(IChargingstation chargingstation, double level)
	{
		lazyInit();
		...
		step[0]	= () ->
		{
			if(!reached.isDone())	// no new timer when future is terminated from outside (e.g. agent killed)
			{
				...
				{
					reached.setException(new IllegalStateException("Cannot not recharge. Charging station out of reach!"));
					return null;
				}
...
//	}
	 ...
	 */
	public <T extends ILocationObject> void updateObjects(Set<T> oldset, T[] newset)
	{
