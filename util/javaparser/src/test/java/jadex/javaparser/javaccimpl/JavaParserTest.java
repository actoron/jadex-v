		...
		for(Iterator<Object> it=props.keySet().iterator(); it.hasNext(); )
		{
			...
			else
			{
				...
				try
				{
					...
					Assertions.assertFalse(parsex, "Expected parse exception");
				}
				catch(Throwable e)
				{
//					e.printStackTrace();
					Assertions.assertTrue(parsex, "Unexpected parse exception: "+e);
	//				if(parsex)
	//				{
	...
	//				}
				...
				if(node!=null && !parsex)
				{
					try
					{
						...
						evaluated	= true;
						Assertions.assertFalse(evalex, "Expected evaluation exception");
					}
					...
					{
						Assertions.assertTrue(evalex, "Unexpected evaluation exception: "+e);
						if(evalex)
						{
							Assertions.assertTrue(e.toString().startsWith(type), "Unexpected evaluation exception type: "+e);
						}
					}
				}
				...
				{
					String	rets	= ""+retval;
					Assertions.assertTrue(rets.startsWith(value), "Unexpected value "+rets+", should be "+value+")");
				}
			}
		}
	}
